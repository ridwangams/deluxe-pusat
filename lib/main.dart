...
Future<void> showWAStyleNotification(String title, String message) async {
  ...
  void initState() {
    ...
    Future.delayed(Duration(milliseconds: 1500), () {
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (_) => WebViewScreen(url: WORKER_URL)),
      );
    });
  }
      ...
      if (response.statusCode == 200) {
        ...
        if (data['show'] == true) {
          if (!isMaintenance) {
            ...
            await showWAStyleNotification(maintenanceTitle, maintenanceMessage);
          } else if (maintenanceMessage != data['message']) {
            setState(() {
              maintenanceMessage = data['message'];
            });
            await showWAStyleNotification(data['title'] ?? "Update", data['message']);
          }
        } else {
